...
Very simply, those elements include a request router, persistent
storage, a direct read path and a cache, a direct write path and a
queue, synchronous request handlers, and asynchronous work handlers.
...
This document is not intended to be a primer or educational.  Instead,
it is a list of bullet points, likely to be covered in a systems
design interview.  My hope is to outline the basics so that they can
be covered quickly and leave more time open to either discuss
individual points in depth, or make time available to discuss novel
and difficult designs.
...
and a larger set of read-only replicas.
...
publishing service relies on a sharded, denormalized, read-only data
store that is constructed by an extract, transform, load (ETL) job.
Data from the publishers' service is exported in a format that is
ideal for quick, low-latency lookups.
